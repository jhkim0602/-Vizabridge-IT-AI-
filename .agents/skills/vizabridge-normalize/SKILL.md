...
2. **Load progress**. Run `python .Codex/skills/vizabridge-normalize/scripts/show_progress.py {manual_key}`. It prints the next pending chunk_id, total pending, and total completed. If everything is done, stop and report.
...
7. **Append**. Run `python .Codex/skills/vizabridge-normalize/scripts/append_normalized.py {manual_key} {chunk_id}` with your block on stdin. The script verifies the chunk_id and hash match the index, that the chunk is not already in the output, and writes atomically.
