...
You emit **one row per (petition_type, subsection_type) you find usable content for**. Some visa codes will yield 3 rows, some 30. Use judgment, but follow the canonical lists below.

## Canonical petition types

Choose `petition_type` from this list. Map source phrases to the canonical value on the left.
...
Stay manual rows typically use the bottom block (체류자격*, 외국인등록 등). Visa manual rows typically use the top block (사증발급*, 초청). Some rows belong to both — pick whichever the surrounding text emphasizes.
...
행마다 한국어 질문 **3개**를 줄바꿈으로 구분해 적는다. 사람 검수자가 "이 행이 어떤 사용자 상황에 답이 되는가"를 한눈에 보기 위한 용도다.
...
- 3개를 다 채울 만큼 본문이 없으면 채울 수 있는 만큼만(1~2개) 적고 줄을 비워두는 것도 허용. 억지로 채우지 말 것.
